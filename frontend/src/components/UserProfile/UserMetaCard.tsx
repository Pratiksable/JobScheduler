...
useEffect(() => {
  const fetchUser = async () => {
    try {
      ...
      const token = localStorage.getItem("token"); // if your backend needs it
  ...
  const handleSave = () => {
    ...
    closeModal();
    // Add PATCH/PUT request here if you want to update backend
  };
                  ...
                  {Object.entries(user).map(([key, value]) => (
                    <div key={key} className="col-span-2 lg:col-span-1">
                      ...
                      <Input
                        type="text"
                        value={value}
                        readOnly={key === "id" || key === "email"} // make ID/email read-only
                        onChange={(e) =>
                          setUser((prev: any) => ({
                            ...prev,
                            [key]: e.target.value,
